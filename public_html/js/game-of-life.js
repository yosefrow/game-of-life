 ...
 */
window.gameOfLife = (function(canvasContainer, dimensions){
    /***********************************************************************************************
    * Settings
    ***********************************************************************************************/
    ...
    settings.aspectRatio = 0; // this is populated correctly in sketch.setup

    /**
     * Adust the refreshRate setting which defines the framerate
     * (This is a setting only. Current instance is unaffected)
     * 
     * @param {number} rate - number of frames per second
     */
    settings.setRefreshRate = function(rate) {
        this.refreshRate = constrain(rate, 1, 100);
    };
        
    /**
     * Adjust the dimensions setting which defines the grid dimensions
     * (This is a setting only. Current instance is unaffected)
     * 
     * @param {number} x - number of cells in the x axis (columns)
     * @param {number} y - number of cells in the y axis (rows)
     */
    settings.setDimensions = function(x, y) {
        var minDimension = 5;
        var maxDimension = 500;

        if (this.dimensions.y !== 'aspectRatio') {
            this.dimensions.x = round(constrain(x, minDimension, maxDimension));
            if (y) {
                this.dimensions.y = round(constrain(y, minDimension, maxDimension));
            }
        }
        else {
            this.dimensions.x = round(
                constrain(x, minDimension / this.aspectRatio, 
                maxDimension / this.aspectRatio)
            );
        }
    ...
    var keyboard;
    var handleKeyPresses;
    var handleLongKeyPresses;

    /**
     * P5 Sketch Setup function, to be assigned to setup global
     */
    sketch.setup = function() {
        ...
        if (settings.dimensions.y === 'aspectRatio') {
            settings.aspectRatio = canvas.size.y / canvas.size.x;
            //console.log(settings.aspectRatio);
        }
        ...
        /*******************************************************************************************
         * Nothing After this point is essential to the game's core)
         ******************************************************************************************/
        /*******************************************************************************************
         * Render Functions (Pure Rendering, no object modifications whatsoever)
         ******************************************************************************************/
        /**
         * Colors Enumerator
         * 
        ...
        /*******************************************************************************************
         * Game Instance Management and Flow Control
         ******************************************************************************************/

        // for some reason redefintions are not properly exported. So we must define each property
        // individually, or the export won't work o_o
        /**
         ...
         * @type {Object}
         */
        game = {}; // re-initialize game for documentation purposes and to show it is unmodified
        game.current = undefined;
        game.playing = false;

        /**
         * Toggle the game.playing state from true or false
         */
        ...
        /**
         * Generate new instance of GameOfLife, pass it dimensions, and attach it to game.current
         *
         * @depends {Object} settings
         */
        game.initialize = function() {
            fps.reset();
            var y = settings.dimensions.y;
            if (y === 'aspectRatio') {
                y = round(settings.dimensions.x * settings.aspectRatio);
            }
            //console.log(y);
            this.current = new GameOfLife(settings.dimensions.x, y);
            this.playing = true;
        };

        /**
         * Go forward one step in the game (get next generation)
         *
         * @depends render
         */
        game.forward = function() {            
            this.current.update();
            render.game(this.current);
        };

        /**
         * Adjust refresh rate (frame rate)
         * @param  {number} amount - target frames per second
         *
         * @depends settings
         */
        game.adjustRefreshRate = function(amount) {
            settings.setRefreshRate(settings.refreshRate + amount);  
            frameRate(settings.refreshRate);
        };

        /**
         * Scale game dimensions and re-initialize game
         * @param  {number} amount - target frames per second
         * 
         * @depends settings
         */
        game.scaleDimensionsBy = function(factor) {
            settings.setDimensions(settings.dimensions.x * factor);
            this.initialize();
        };

        /**
         * Zoom in table - reduce number of cells, but increase their size
         */
        game.zoomIn = function() {
            this.scaleDimensionsBy(0.9);
        };

        /**
         * Zoom out table - increase number of cells, but decrease their size
         */
        game.zoomOut = function() {
            this.scaleDimensionsBy(1.1);
        };

        /**
         * Speed up game - increase refreshRate (frames per second)
         * (actual frames per second capped by machine limits)
         */
        game.speedUp = function() {
            this.adjustRefreshRate(+1);
        };

        /**
         * slow down game - decrease refreshRate (frames per second)
         * (actual frames per second capped by machine limits)
         */
        ...
        performance = {
            optimizing: true,
            /**
             * After first frame average is complete, estimate the performance lag and compensate
             */
            optimize: function() {
                ...
                if (frameCount <= fps.framesToAverage) {
                    if (frameCount == fps.framesToAverage && 
                        ...
                        game.scaleDimensionsBy(performanceFactor);
    ...
    /**
     * P5 Sketch Draw function, to be assigned to draw global
     */
    ...
    /**
     * P5 Sketch mouseClicked function, to be assigned to mouseClicked global
     */
    ...
    /**
     * P5 Sketch keyPressed function, to be assigned to keyPressed global
     */
    ...
    /**
     * P5 Sketch keyReleased function, to be assigned to keyReleased global
     */
     ...
     * at a time. For real p5 instances we can use the built in p5.js instancing
     */
...
gameOfLife.sketch.load(); //jshint ignore:line